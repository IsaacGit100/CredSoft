...
from .models import Master
from .forms import MasterForm, MasterSearchForm
...
def members_home(request):
    ...
    return render(request, 'MembersApp/member_home.html')
...
def member_detail(request, slug, pk):
    ...
    return render(request, 'MembersApp/member_detail.html', context)
...
@login_required
def updated_member_list(request, slug):
    ...
    return render(request, "MembersApp/updated_member_list.html", context)
...
def member_create(request, slug):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Get NOK percentages for verification (if you have these fields)
    ...
    context = {
        ...
        'cancel_url': "MembersApp:member_list_manage', slug=entity.slug",      
    }
    return render(request, 'MembersApp/member_create.html', context)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                return redirect(
                    "MembersApp:member_view", slug=slug, pk=member.id)
...
def members_sav_int_list(request, slug):
    ...
    return render(request, 'MembersApp/members_sav_int_list.html', context)
